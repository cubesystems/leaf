...
function replacePngTags($x,$img_path='',$sizeMeth='scale',$inScript=FALSE){
    ...
    for($num_images=0;$num_images<count($images[0]);$num_images++){
		...
		$replaceFrom = $quote . $images[3][$num_images] . $quote;
        ...
        if($width==0 || $height==0){
            ...
            if(file_exists(realpath(PATH.$img_path.$images[3][$num_images]))){
                // image is on this filesystem, get width & height
                $size=getimagesize(PATH.$img_path.$images[3][$num_images]);
                $width=$size[0].'px';
                $height=$size[1].'px';
            }else if(file_exists(PATH.$images[3][$num_images])){
                // image is on this filesystem, get width & height
                $size=getimagesize(PATH.$images[3][$num_images]);
                $width=$size[0].'px';
                $height=$size[1].'px';
				$images[3][$num_images] = WWW . $images[3][$num_images];
            }
			else if(file_exists($imagePath = str_replace(WWW, PATH, $images[3][$num_images]))){
                // image is on this filesystem, get width & height
        ...
        $new_tag=str_replace($replaceFrom, $replace_src_with,
